...
def train_model(model, x_train, x_test, y_train, y_test, model_name, track_index):
    ...
    if mutate_cfgs['do_mutate']:
        model_folder = os.path.join(mutate_cfgs['mutate_dir'], ((((mutate_cfgs['mutate_func'] + '_') + mutate_cfgs['mutate_func_params']['make_output_classes_overlap']) + '_') + mutate_cfgs['mutate_func_params']['layer']))

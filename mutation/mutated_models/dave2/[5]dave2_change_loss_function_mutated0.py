...
def train_model(model, x_train, x_test, y_train, y_test, model_name, track_index):
    ...
    model_folder = os.path.join(mutate_cfgs['mutate_dir'], ('change_loss_function_' + mutate_cfgs['mutate_func_params']['new_loss_function']))
    ...
    model.compile(loss=loss_operators.operator_change_loss_function(mutate_cfgs['mutate_func_params']['new_loss_function']), optimizer=Adam(lr=Training_Configs['LEARNING_RATE']))
...
def main():
    ...
    # os.chdir(os.getcwd())
    # import sys
    # track_index = sys.argv[1]  # 第一个参数
    # print(f"Received value: {track_index}")

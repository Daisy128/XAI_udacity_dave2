...
def train_model(model, x_train, x_test, y_train, y_test, model_name, track_index):
    ...
    model_folder = os.path.join(mutate_cfgs['mutate_dir'], ('add_weights_regularisation_' + mutate_cfgs['mutate_func_params']['weights_regularisation'] + '_' + str(mutate_cfgs['mutate_func_params']['weights_regularisation_strength'])))
    ...
    model.compile(loss='mse', optimizer=Adam(lr=Training_Configs['LEARNING_RATE']))

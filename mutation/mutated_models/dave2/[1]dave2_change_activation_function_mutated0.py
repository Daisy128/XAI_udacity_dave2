...
def train_model(model, x_train, x_test, y_train, y_test, model_name, track_index):
    ...
    model_folder = os.path.join(mutate_cfgs['mutate_dir'], ('change_activation_function_' + mutate_cfgs['mutate_func_params']['new_activation_function']) + '_4-9')
    ...
    early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=.0001, patience=2, mode='auto')

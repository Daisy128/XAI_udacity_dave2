...
def get_generators(x_train, x_test, y_train, y_test):
    ...
    x_train: 'x_train'
    y_train: 'y_train'
    (x_train, y_train) = training_data_operators.operator_make_output_classes_overlap(x_train, y_train, properties.make_output_classes_overlap['make_output_classes_overlap_pct'])
...
def train_model(model, x_train, x_test, y_train, y_test, model_name, track_index):
    ...
    print("track index is : ", track_index)
    track_info = track_infos[track_index]
    current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
    model_folder = os.path.join(mutate_cfgs['mutate_dir'], ('make_output_classes_overlap_' + str(mutate_cfgs['mutate_func_params']['make_output_classes_overlap_pct'])))

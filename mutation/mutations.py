...
class Mutation():
    applyOnce = True #是否在找到第一个符合条件的节点后停止，还是继续对所有符合条件的节点逐一变异
    mutationName = None
...
#TODO: Check if we need this method
    ...
    def mutate(self, file_path, save_path_mutated):
        ...
        try:
            with open(file_path, "r") as source:
                tree = ast.parse(source.read()) # ast抽象语法树，读取并解析file_path的代码
        except Exception as e:
            logger.error("Mutation.mutate parse AST tree failed" + str(e))
            raise

        try:
            was_annotated = self.mutate_annotated(tree, save_path_mutated) # was_annotated is False
        except Exception as e:
    ...
    def mutate_automatically(self, tree, save_path_mutated):
        ...
        Returns: -
        """
        # suffix starts from 0, if self.appliOnce == False, increase each time
        application_cnt = 0
        # Look for the right place to insert the mutation
        for node in ast.walk(tree):
            if hasattr(node, 'body') and isinstance(node.body, list):
...
class ChangeActivationAFMut(Mutation):
    ...
    def generate_mutation_node(self, elem, model_params_ann = None):
        ...
        # return a statement: model_name = module_name.operator_name(model_name)

import os.path

from perturbationdrive.perturbationdrive_new import PerturbationDrive
from perturbationdrive.RoadGenerator.CustomRoadGenerator import CustomRoadGenerator
from udacity_gym import UdacitySimulator, UdacityGym
from perturbationdrive.RoadGenerator.dave2_agent import Dave2Agent
import traceback
from udacity_gym.agent_tf import SupervisedAgent_tf


# def run_perturb_road_generator(config):
#
#
#     road_angles_list=[[-37, 44, 53, -60, 69, 17, -1, -32, -43, 57],
#                       [-52, 41, -20, 21, 63, 50, -22, -29, 8, -6],
#                       [8, 27, -35, -2, -28, -2, -13, 49, 2, 11],
#                       [35, -20, -23, 9, -24, -14, 43, 26, -23, -3],
#                       [9, -9, -5, -26, -28, -20, 9, -27, 5, -34],
#                       [36, 32, 28, 24, 20, 16, 12, 8, 4, 0],
#                       [-36, -32, -28, -24, -20, -16, -12, -8, -4, 0],
#                       [28, 24, 20, 16, 12, 8, 4, 0, -4, -8],
#                       [-28, -24, -20, -16, -12, -8, -4, 0, 4, 8],
#                       [35, 35, 35, 35, 35, 35, 35, 35],
#                       [-35, -35, -35, -35, -35, -35, -35, -35],
#                       [25, 25, 25, 25, 25, 25, 25, 25],
#                       [-25, -25, -25, -25, -25, -25, -25, -25],
#                       [45, -45, 45, -45, 45, -45, 45, -45, 45, -45],
#                       [35, -35, 35, -35, 35, -35, 35, -35, 35, -35]]
#     road_segments_list=[[20, 22, 29, 24, 23, 21, 26, 27, 21, 20], [20, 26, 21, 21, 23, 20, 23, 29, 24, 27], [27, 28, 22, 20, 30, 25, 23, 23, 29, 26], [22, 29, 26, 27, 30, 27, 24, 25, 20, 27], [25, 21, 28, 20, 30, 21, 26, 29, 25, 30], [20, 20, 20, 20, 20, 20, 20, 20, 20, 20], [20, 20, 20, 20, 20, 20, 20, 20, 20, 20], [20, 20, 20, 20, 20, 20, 20, 20, 20, 20], [20, 20, 20, 20, 20, 20, 20, 20, 20, 20], [10, 10, 10, 10, 10, 10, 10, 10], [10, 10, 10, 10, 10, 10, 10, 10], [10, 10, 10, 10, 10, 10, 10, 10], [10, 10, 10, 10, 10, 10, 10, 10], [20, 20, 20, 20, 20, 20, 20, 20, 20, 20], [20, 20, 20, 20, 20, 20, 20, 20, 20, 20]]
#     road_types_list=['random', 'random', 'random', 'random', 'random', 'curvy', 'curvy', 'curvy', 'curvy', 'simple', 'simple', 'simple', 'simple', 'swiggly', 'swiggly']
#
#     try:
#         simulator = UdacitySimulator(
#             simulator_exe_path="./udacity/road_generator_simulator/binary.x86_64",
#             host="127.0.0.1",
#             port=9091
#         )
#
#         model_name = "roadGen_trained.h5"
#         agent = Dave2Agent(model_path = os.path.join("./model/ckpts/ads", model_name))
#         model = agent.model
#         attention_map = {
#             "map": "grad_cam",
#             "model": model,
#             "threshold": 0.1,
#             "layer": "conv2d_5",
#         }
#
#         functions = ["posterize_filter", "object_overlay", "reflection_filter", "white_balance_filter"] #, "reflection_filter", "white_balance_filter"]
#
#         benchmarking_obj = PerturbationDrive(
#             simulator,
#             agent,
#             model_name,
#             functions,
#             attention_map,
#             image_size=(160, 320),
#             max_scale=10,
#             start_scale=0,
#             monitor=True,
#             perturb=True
#         )
#
#         for i in range(10, len(road_angles_list)):
#             road_generator = CustomRoadGenerator(num_control_nodes=len(road_angles_list[i]))
#             print(f"{5 * '#'} Testing road {i} {5 * '#'}")
#
#             benchmarking_obj.grid_seach(
#                 road_number=i, # in order to store data
#                 road_generator=road_generator,
#                 road_angles=road_angles_list[i],
#                 road_segments=road_segments_list[i],
#                 test_model=True,
#             )
#             print(f"{5 * '#'} Finished Testing road {i} {5 * '#'}")
#     except Exception as e:
#         print(
#             f"{5 * '#'} Udacity Error: Exception type: {type(e).__name__}, \nError message: {e}\nTract {traceback.print_exc()} {5 * '#'} "
#         )

def run_perturb_udacity_tracks(config, track_conf, perturb_cfgs):

    simulator = UdacitySimulator(
        sim_exe_path=track_conf[config['track_index']]['simulator']['exe_path'],
        host=track_conf[config['track_index']]['simulator']['host'],
        port=track_conf[config['track_index']]['simulator']['port'],
    )

    agent = SupervisedAgent_tf(
        model_path=config['model_path'],
        max_speed=40,
        min_speed=6,
        predict_throttle=True
    )

    benchmarking_obj = PerturbationDrive(
        simulator=simulator,
        agent=agent,
        model=config['model_name'],
        perturbation_functions=config['perturbations'],
        attention_map={},
        image_size=(160, 320),
        max_scale=perturb_cfgs['max_scale'],
        start_scale=config['start_scale'],
        perturb=True,
    )

    track_name = track_conf[config['track_index']]['track_name']
    benchmarking_obj.perturb_tracks(
        track_name = track_name,
        daytime = "day",
        weather = "sunny",
        low_speed_threshold = perturb_cfgs['low_speed_threshold'],
        low_speed_limit = perturb_cfgs['low_speed_limit'],
        total_crash_limit = perturb_cfgs['total_crash_limit'][track_name],
        log_dir=perturb_cfgs['log_dir'],
    )